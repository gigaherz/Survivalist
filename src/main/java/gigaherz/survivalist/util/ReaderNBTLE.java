    ...
    private static INBT readSpecific(FileInputStream input, byte type) throws IOException
    {
        switch (type)
        {
            ...
            case Constants.NBT.TAG_BYTE:
                return ByteNBT.valueOf(readByte(input));
            case Constants.NBT.TAG_SHORT:
                return ShortNBT.valueOf(readShort(input));
            case Constants.NBT.TAG_INT:
                return IntNBT.valueOf(readInt(input));
            case Constants.NBT.TAG_LONG:
                return LongNBT.valueOf(readLong(input));
            case Constants.NBT.TAG_FLOAT:
                return FloatNBT.valueOf(Float.intBitsToFloat(readInt(input)));
            case Constants.NBT.TAG_DOUBLE:
                return DoubleNBT.valueOf(Double.longBitsToDouble(readLong(input)));
            case Constants.NBT.TAG_STRING:
                return StringNBT.valueOf(readString(input));
